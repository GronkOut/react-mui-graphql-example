...
export default function PagesTemplateList() {
  ...
  const {
    ...
    data: templatesData,
    refetch,
  } = useQuery<TemplatesByContentIdData>(GET_TEMPLATES_BY_CONTENT_ID, {
  ...
  const handleRowClick: GridEventListener<'rowClick'> = useCallback(({ row }) => navigate(`/content/${contentId}/${row.id}`), [navigate, contentId]);

  const handleClickCreate = useCallback(() => navigate(`/content/${contentId}/create`), [navigate, contentId]);

  if (loading) {
    return <LoadingIndicator />;
  }

  if (!templatesData?.templatesByContentId) {
    return null;
  }

  const { templatesByContentId: templates } = templatesData;
